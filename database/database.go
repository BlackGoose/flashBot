...
func Create(db *sqlx.DB, front, back string, user_id int) error {
...
func Get(db *sqlx.DB, user_id int) ([]Card, error) {
	cards := []Card{}
	err := db.Select(&cards, "select * from cards where user_id = $1 and date_expired <= $2", user_id, time.Now())
	return cards, err
}

func Delete(db *sqlx.DB, front string, user_id int) error {
